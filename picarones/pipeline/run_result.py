...
Phase 5.1 audit code-quality (2026-05) : module déplacé depuis
``picarones.app.results`` vers ``picarones.pipeline.run_result``
pour résoudre 4 imports illégaux ``reports/ → app/`` qui violaient
le contrat d'orientation des couches (``reports/`` est couche 7,
``app/`` couche 6 — l'ordre interdit que reports importe d'une
couche plus externe que pipeline=4).

``app/results.py`` est conservé comme re-export pour ne pas casser
les callers internes existants ; à terme (Phase 12, release v2.0),
le shim peut disparaître.
...
**présentation**, pas du domain.  Le rapport HTML calcule ses
agrégats à la volée depuis les ``ViewResult`` listés.
"""
